...
kubectl exec -n $NS $R1 -- sh -c 'kill -STOP $(pidof ospfd) && echo OSPFD-FROZEN' 2>&1
echo "ospfd FROZEN on r1 (SIGSTOP: no LSA flush, hellos just stop, links UP) at $(date -u +%Y-%m-%dT%H:%M:%S)" | tee -a $OUT/ospf-chaos.log
...
echo "=== RESTORE (SIGCONT ospfd on r1) ===" | tee -a $OUT/ospf-chaos.log
kubectl exec -n $NS $R1 -- sh -c 'kill -CONT $(pidof ospfd) || /usr/lib/frr/frrinit.sh start' 2>&1 | tail -1
sleep 60
