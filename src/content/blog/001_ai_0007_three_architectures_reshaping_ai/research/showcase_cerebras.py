#!/usr/bin/env python3
"""Showcase Recording: Cerebras LPU (Wafer-Scale) - REAL DATA, LONG RUN
   Real call to openai/gpt-oss-120b via OpenRouter, routed exclusively
   to the Cerebras provider (extra_body={"provider": {"only": ["cerebras"]}}).
   Extended ~3000-word prompt + max_tokens=8000 to produce a longer
   generation for a more dramatic race visual.

   LIGHT MODE terminal styling (white background, dark text) - redo of the
   earlier dark-mode terminal GIF, and one combined run also feeds the
   race-comparison animation data.

   Capture with:
     asciinema rec --window-size 150x40 --command "python3 showcase_cerebras.py" demo_cerebras.cast
     agg demo_cerebras.cast demo_cerebras.gif   (after injecting light theme)
...
# Same tokenizer used across all three showcases (Cerebras/DeepInfra/Mercury)
# for an apples-to-apples token count / TPS comparison.
...
# Extended ~3000-word prompt (longer generation than the earlier 1500-word run)
PROMPT = (
...
LOG_FILE = f"cerebras_showcase_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
log_lines = []
...
log(f"Cerebras LPU showcase started | model={MODEL} | provider={PROVIDER_ONLY} | max_tokens={MAX_TOKENS}")

# ── Prompt panel (light mode, shown immediately) ───────────────────────────
console.print(Panel(
    Text(PROMPT, style="bold #1a1a1a"),
    title="[bold #2a2a2a] PROMPT — CEREBRAS LPU (Wafer-Scale, REAL) [/]",
    border_style="#5f5f5f",
    style="on #ffffff",
))
...
console.print(Panel(
    summary,
    title="[bold white on #2a2a2a] GENERATION COMPLETE — CEREBRAS LPU (REAL) [/]",
    border_style="#2a2a2a",
    style="on #ffffff",
))

log(f"AGGREGATE: TTFT={ttft_ms}ms TOTAL={total_s}s TOKENS={token_count} AVG_TPS={avg_tps}")
log("Cerebras LPU showcase completed")
...
with open("results_cerebras_showcase.json", "w") as f:
    json.dump(results, f, indent=2)
with open("results_cerebras_showcase_full_response.txt", "w") as f:
    f.write(full_text)
