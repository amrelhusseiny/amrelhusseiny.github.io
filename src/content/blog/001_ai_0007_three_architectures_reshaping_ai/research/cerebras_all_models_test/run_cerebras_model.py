#!/usr/bin/env python3
"""Cerebras multi-model showcase: real call to a chosen model via OpenRouter,
routed exclusively to the Cerebras provider. Same prompt/settings used
across all models for an apples-to-apples "same hardware, different model"
comparison (complementary to the earlier "same model, different hardware"
Cerebras-vs-DeepInfra test).

Usage:
    python3 run_cerebras_model.py --model openai/gpt-oss-120b --label gpt-oss-120b
    python3 run_cerebras_model.py --model google/gemma-4-31b-it --label gemma-4-31b
    python3 run_cerebras_model.py --model z-ai/glm-4.7 --label glm-4.7
"""
...
# Identical prompt used across all Cerebras runs for apples-to-apples comparison
PROMPT = (
...
print(f"Provider: Cerebras (via OpenRouter)")
...
with open(f"results_cerebras_{args.label}.json", "w") as f:
    json.dump(result, f, indent=2)
with open(f"response_cerebras_{args.label}.txt", "w") as f:
    f.write(full_text)
